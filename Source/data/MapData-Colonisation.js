...
var canonnEd3d_route = {
	...
	systemsData: {
		categories: {
			'Systems': {
				...
				'07': {
					name: 'Generation 1 Route',
					color: "00ff00",
				},
				'08': {
					name: 'Generation 1 Dead End',
					color: "ff0000",
				},
				'Generation': {
					'07': {
						name: 'Generation 1 Route',
						color: "303030",  // You can change this color
					},
				},
			},
		},
	...
	formatCol: function (factionData, homeSystem) {
		...
		polSite = {}
		polSite['cat'] = ['01'];
		polSite['name'] = 'Polaris';
		polSite['infos'] = 'Permit Locked System<br>'
		polSite['coords'] = {
			x: -322.6875,
			y: 194.59375,
			z: -212.4375,
		};
		canonnEd3d_route.systemsData.systems.push(polSite);
	...
	parseGenRoutes: function (url, resolvePromise) {
		let fetchDataFromApi = async (url, resolvePromise) => {
			let response = await fetch(url);
			let result = await response.json();

			result.forEach(route => {
				// Only add systems that don't already exist in the lookup
				if (!canonnEd3d_route.systemLookup[route.start.name]) {
					let startSystem = {
						cat: ['07'],  // Generation 1 category
						name: route.start.name,
						coords: {
							x: parseFloat(route.start.x),
							y: parseFloat(route.start.y),
							z: parseFloat(route.start.z)
						}
					};
					//canonnEd3d_route.systemsData.systems.push(startSystem);
					//canonnEd3d_route.systemLookup[route.start.name] = startSystem.coords;
				}
				...
				let routeData = {
					cat: ['06'],
					'points': [
						{ 's': route.start.name, 'label': route.start.name },
						{ 's': route.end.name, 'label': route.end.name }
					],
					'circle': false
				};

				canonnEd3d_route.systemsData.routes.push(routeData);
			});
	...
	init: function () {
		...
		var p3 = new Promise(function (resolve, reject) {
			canonnEd3d_route.parseGenRoutes('data/csvCache/gen1_routes.json', resolve)
		});
