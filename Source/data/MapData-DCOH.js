...
function renderSystem(f) {
	maelstrom = '<b>' + f.maelstrom.name + '<b></br>'
	progress = '<b>Progress</b>: ' + f.progress + '</br>'
	stationsUnderAttack = '<b>Stations Under Attack</b>: ' + f.stationsUnderAttack + '</br>'
	stationsUnderRepair = '<b>Stations Under Repair</b>: ' + f.stationsUnderRepair + '</br>'

	return maelstrom + progress + stationsUnderAttack + stationsUnderRepair
}
...
var canonnEd3d_route = {
	...
	parseApi: function (url, resolvePromise) {
		let fetchDataFromApi = async (url, resolvePromise) => {
			let response = await fetch(url);
			let result = await response.json();
			canonnEd3d_route.dcohData = result
			resolvePromise();
			return result;
		}
	...
	systemLookup: {},
	formatCol: function (dcohData) {
		//Here you format POI & Gnosis JSON to ED3D acceptable object

		data = dcohData.systems
		...
		for (var i = 0; i < data.length; i++) {
			...
			poiSite['cat'] = [data[i].thargoidLevel.level]



			poiSite['coords'] = {
				x: parseFloat(data[i].coordinates.x),
				y: parseFloat(data[i].coordinates.y),
				z: parseFloat(data[i].coordinates.z),
			};
	...
	init: function () {
		...
		var p1 = new Promise(function (resolve, reject) {
			...
			canonnEd3d_route.parseApi('https://dcoh.watch/api/v1/overwatch/systems?ngsw-bypass=true', resolve)
		});
		...
		Promise.all([p1]).then(function () {
			...
			canonnEd3d_route.formatCol(canonnEd3d_route.dcohData)
			...
			Ed3d.init({
				...
				cameraPos: [canonnEd3d_route.camerapos.x - 400, canonnEd3d_route.camerapos.y, canonnEd3d_route.camerapos.z - 400],
