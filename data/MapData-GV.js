...
var canonnEd3d_gv = {
	...
	formatSitesStream: async function (siteDataBatch) {
		...
		for (var d = 0; d < siteDataBatch.length; d++) {
			if (siteDataBatch[d].system.systemName && siteDataBatch[d].system.systemName.replace(' ', '').length > 1) {
				...
				if (siteDataBatch[d].type.type == 'Sulphur Dioxide Gas Vent') {
					poiSite['cat'] = [201];
				} else if (siteDataBatch[d].type.type == 'Water Gas Vent') {
					poiSite['cat'] = [202];
				} else if (siteDataBatch[d].type.type == 'Carbon Dioxide Gas Vent') {
					poiSite['cat'] = [203];
				} else if (siteDataBatch[d].type.type == 'Silicate Vapour Gas Vent') {
					poiSite['cat'] = [204];
				} else {
					poiSite['cat'] = [2000];
				}
	...
	init: function () {
		...
		streamAllSites(canonnEd3d_gv.formatSitesStream.bind(canonnEd3d_gv));
	},
};
