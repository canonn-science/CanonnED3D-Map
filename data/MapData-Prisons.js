...
var canonnEd3d_route = {
	...
	systemsData: {
		categories: {
			'Systems': {
				'00': { name: 'Prisons', color: getColour(1), },
				'01': { name: 'Stations', color: getColour(5), }

			},
			"Routes": {
				"08": { name: 'Prison Jumps', color: getColour(9), }
			}
		},
		systems: [],
		routes: [],
	...
	formatRoute: function (data) {
		...
		for (var i = 0; i < data.length - 1; i++) {
			...
			route = {
				cat: cat,
				'points': [
					{ 's': start.name, 'label': start.name },
					{ 's': end.name, 'label': end.name }], 'circle': false
			}
	...
	formatSystems: function (routes) {

		prisons = {}


		routes.forEach(function (route_data) {
			/*poiSite = [];


			poiSite['cat'] = ["00"];
			poiSite['name'] = route_data.prison.name
			poiSite['coords'] = {
				x: parseFloat(route_data.prison.coords.x),
				y: parseFloat(route_data.prison.coords.y),
				z: parseFloat(route_data.prison.coords.z),
			};
			canonnEd3d_route.systemsData.systems.push(poiSite);
*/

			prisons[route_data.prison.name] = route_data;

			poiSite = [];

			poiSite['cat'] = ["01"];
			poiSite['name'] = route_data.jump.name
			poiSite['coords'] = {
				x: parseFloat(route_data.jump.coords.x),
				y: parseFloat(route_data.jump.coords.y),
				z: parseFloat(route_data.jump.coords.z),
			};
			canonnEd3d_route.systemsData.systems.push(poiSite);
		...
		console.log(prisons)
		for (var key in prisons) {
			console.log("looping");
			poiSite = [];
			console.log(route_data)
			prison = prisons[key].prison
			poiSite['cat'] = ["00"];
			poiSite['name'] = prison.name
			poiSite['coords'] = {
				x: parseFloat(prison.coords.x),
				y: parseFloat(prison.coords.y),
				z: parseFloat(prison.coords.z),
			};
			canonnEd3d_route.systemsData.systems.push(poiSite);
		};
	...
	init: function () {
		...
		var p1 = new Promise(function (resolve, reject) {
			canonnEd3d_route.parseData('data/station_stats.json', resolve)
